...
export function SpamVerificationCard({ message, onClick }) {
    ...
    useEffect(() => {
        if (subjectCode)
            return; // already got it from subject
        let cancelled = false;
    ...
    return (_jsxs("div", { className: styles.card, onClick: onClick, role: "button", tabIndex: 0, children: [_jsx("div", { className: styles.from, children: fromLabel }), _jsx("div", { className: styles.label, children: "Verification code" }), _jsx("div", { className: styles.digits, children: code.split('').map((d, i) => (_jsx("span", { className: styles.digit, children: d }, i))) }), _jsx("button", { onClick: copy, style: {
                ...
                }, children: copied ? '✓ Copied' : 'Copy code' }), _jsx("div", { className: styles.subject, children: message.subject })] }));
}
