...
import type { Message, Bowl } from '../types'
...
export function GlobalSearch({ bowls, compact = false }: Props) {
  ...
  function bowlFor(message: Message): Bowl | undefined {
  ...
  return (
    <>
      {/* Trigger button — pill on desktop, icon button on mobile */}
      {compact ? (
        <button
        ...
        <button
          ...
          onMouseOver={e => (e.currentTarget.style.borderColor = 'var(--border-med)')}
          onMouseOut={e => (e.currentTarget.style.borderColor = 'var(--border)')}
        >
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <circle cx="11" cy="11" r="8" /><path d="m21 21-4.3-4.3" />
          </svg>
          <span style={{ flex: 1, textAlign: 'left' }}>Search across all inboxes…</span>
          <span style={{
        ...
        >
          ...
          <div style={{
            ...
            <input
              ...
              placeholder="Search across all bowls…"
              style={{
            ...
            />
            {searching && (
              <span style={{ fontSize: '0.7rem', color: 'var(--text-3)' }}>Searching…</span>
            )}
            <button
              onClick={() => setOpen(false)}
              style={{
          ...
          <div style={{ maxHeight: '60vh', overflowY: 'auto' }}>
            {query.length < 2 && (
              <div style={{ padding: '2rem', textAlign: 'center', color: 'var(--text-3)', fontSize: '0.85rem' }}>
                Start typing to search subjects, senders, and previews…
              </div>
            )}
      ...
      {/* Message viewer for clicked result */}
      {selected && (
        <MessageViewer
          message={selected}
          accentColor={bowlFor(selected)?.color ?? '#94a3b8'}
